...
Customer surfaces are `/my-account/coupon/`, the **Coupon** account-menu item, `[lwc_promo_dashboard]`, cart/checkout promo cards, and Checkout Block selection. The dashboard requires login, queries marked LoveCatz promos, evaluates eligibility/expiry/usage, and renders active or disabled cards. WooCommerce performs final application validation. Missing artwork uses `assets/2026_VOUCHER-REORDER_FINAL.webp`.

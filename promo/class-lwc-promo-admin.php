...
class LWC_Promo_Admin {
		...
		<div class="lwc-promo-manager">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="lwc-promo-form">
				...
				<div class="lwc-promo-form-grid">
					...
					<label><span><?php esc_html_e( 'Eligible users', 'lovecatz-wc' ); ?></span><?php $this->render_eligible_user_select( $values['eligible_user_ids'] ); ?><small><?php esc_html_e( 'Default: all users. Select specific accounts only for a private promo; leave every option unselected for a global coupon.', 'lovecatz-wc' ); ?></small></label>
					<label><span><?php esc_html_e( 'Total usage limit (all users)', 'lovecatz-wc' ); ?></span><input type="number" name="usage_limit" min="1" step="1" value="<?php echo esc_attr( $values['usage_limit'] ); ?>" /><small><?php esc_html_e( 'Set to 1 to let the coupon be used only once across the entire store. Leave empty for unlimited total uses.', 'lovecatz-wc' ); ?></small></label>
					<label><span><?php esc_html_e( 'Usage limit per user', 'lovecatz-wc' ); ?></span><input type="number" name="usage_limit_per_user" min="1" step="1" value="<?php echo esc_attr( $values['usage_limit_per_user'] ); ?>" /><small><?php esc_html_e( 'Set to 1 so every user can use this global coupon once. Leave empty for no per-user limit.', 'lovecatz-wc' ); ?></small></label>
	...
	public function save_coupon() {
		...
		if ( $id && ! $coupon->get_id() ) { wp_die( esc_html__( 'Invalid promo coupon.', 'lovecatz-wc' ) ); }
		...
		$amount = 'lwc_free_shipping' === $type ? 0 : ( 'fixed_cart' === $type ? ( $_POST['fixed_amount'] ?? 1 ) : ( $_POST['percentage_amount'] ?? 1 ) );
		$user_ids = isset( $_POST['eligible_user_ids'] ) && is_array( $_POST['eligible_user_ids'] ) ? $this->normalize_user_ids( wp_unslash( $_POST['eligible_user_ids'] ) ) : array();
		$emails = array(); foreach ( $user_ids as $user_id ) { $user = get_userdata( $user_id ); if ( $user ) { $emails[] = $user->user_email; } }
		$coupon->set_code( $code );
		$coupon->set_discount_type( $type );
		$coupon->set_amount( 'lwc_free_shipping' === $type ? 0 : max( 1, absint( $amount ) ) );
		...
		update_post_meta( $id, '_lwc_promo_maximum_discount', in_array( $type, array( 'percent', 'lwc_free_shipping' ), true ) && ! empty( $_POST['maximum_discount'] ) ? absint( wp_unslash( $_POST['maximum_discount'] ) ) : '' );
	...
	public function delete_coupon() { $this->assert_permission( 'lwc_delete_promo_coupon' ); $id = isset( $_GET['coupon_id'] ) ? absint( $_GET['coupon_id'] ) : 0; if ( $id && $this->is_valid_coupon_post( $id ) ) { wp_trash_post( $id ); } wp_safe_redirect( add_query_arg( array( 'page' => 'lovecatz-wc', 'tab' => 'promo' ), admin_url( 'admin.php' ) ) ); exit; }
	...
	/** Render a genuine multi-select; wp_dropdown_users() does not add multiple. */
	private function render_eligible_user_select( $selected ) {
		...
		echo '<select name="eligible_user_ids[]" id="lwc_promo_eligible_user_ids" class="lwc-promo-user-select" multiple size="6">';
		foreach ( $users as $user ) {
			$label = sprintf( '%1$s (%2$s)', $user->display_name, $user->user_email );
			echo '<option value="' . esc_attr( $user->ID ) . '"' . selected( in_array( (int) $user->ID, $selected, true ), true, false ) . '>' . esc_html( $label ) . '</option>';
		}
		echo '</select>';
	}
	private function render_coupon_list() { $posts = get_posts( array( 'post_type' => 'shop_coupon', 'post_status' => 'publish', 'numberposts' => -1, 'orderby' => 'title', 'order' => 'ASC', 'meta_key' => '_lwc_promo_created', 'meta_value' => '1' ) ); if ( ! $posts ) { echo '<p>' . esc_html__( 'No promo coupons have been created.', 'lovecatz-wc' ) . '</p>'; return; } echo '<table class="widefat striped"><thead><tr><th>' . esc_html__( 'Code', 'lovecatz-wc' ) . '</th><th>' . esc_html__( 'Discount', 'lovecatz-wc' ) . '</th><th>' . esc_html__( 'Expiry', 'lovecatz-wc' ) . '</th><th></th></tr></thead><tbody>'; foreach ( $posts as $post ) { $coupon = new WC_Coupon( $post->ID ); $discount = $this->format_coupon_discount( $coupon ); $edit = add_query_arg( array( 'page' => 'lovecatz-wc', 'tab' => 'promo', 'coupon_id' => $post->ID ), admin_url( 'admin.php' ) ); $delete = wp_nonce_url( add_query_arg( array( 'action' => 'lwc_delete_promo_coupon', 'coupon_id' => $post->ID ), admin_url( 'admin-post.php' ) ), 'lwc_delete_promo_coupon', 'lwc_promo_nonce' ); echo '<tr><td><strong>' . esc_html( $coupon->get_code() ) . '</strong></td><td>' . esc_html( $discount ) . '</td><td>' . esc_html( $coupon->get_date_expires() ? $coupon->get_date_expires()->date_i18n( get_option( 'date_format' ) ) : '—' ) . '</td><td><a href="' . esc_url( $edit ) . '">' . esc_html__( 'Edit', 'lovecatz-wc' ) . '</a> · <a class="submitdelete" href="' . esc_url( $delete ) . '">' . esc_html__( 'Trash', 'lovecatz-wc' ) . '</a></td></tr>'; } echo '</tbody></table>'; }

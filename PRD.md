...
Last code review: 2026-09-25
Current plugin version: 1.0.88
...
- **Uninstall** removes everything: all four account tables (including legacy), every `lwc_*` option, `lwc_*` transients plus timeouts, `_lwc_*` post meta and HPOS order meta, the `lwc_customer_id` user meta (WooCommerce's own address fields are untouched), and generated legacy `fedex-label-*.pdf` plus current `YYYYMM_LABEL_FEDEX_Customer Name.pdf` files in uploads.
...
The sidebar menu expands into one submenu entry per main tab (Setting, Products, Order, Members, Shipping, Promo, Currency); each links straight to that tab via `?page=lovecatz-wc&tab=…`, and the open tab highlights its entry through a `submenu_file` filter.
- **Products** — quantity-limit and pre-order feature controls.
- **Order** — custom order ID (prefix, date, scope label, running number, suffix) per destination scope.
...
The **Order** tab (`lwc_order_options`) replaces WooCommerce's numeric order number with a readable ID. It is assembled as prefix, date, scope label, running number, and optional suffix, joined by a configurable separator (default `_`), for example `SBI-DD_20260923_Local_0001`.

One master switch (`lwc_order_number_enabled`) turns the feature on; a second switch (`lwc_order_number_reset_daily`) restarts the running number every day. Scope is derived from the destination: shipping country `ID` is **Local**, every other country is **Global**. When no shipping country is stored, the billing country decides, and the store base country is the last resort.

Each scope owns a prefix, a PHP date format (empty omits the date), a scope label (`Local`/`Global` by default), a suffix, a zero-padding length, a start number, and its own counter option (`lwc_order_number_seq_{scope}`, suffixed with `Ymd` while daily reset is on). The counter is incremented with a single SQL statement so concurrent checkouts cannot receive the same number. Raising the start number lifts the next number; lowering it never reuses numbers.

`LWC_Order_Number` covers every order-creation path. The primary hook is `woocommerce_before_order_object_save`, which fires for every `WC_Order::save()` call (create + update) in both HPOS and CPT, so admin "Add new order", programmatic `wc_create_order()`, checkout, and any third-party importer that goes through `WC_Order::save()` all land on the custom mechanism. `woocommerce_new_order` and `woocommerce_process_shop_order_meta` are kept as defensive duplicates. `woocommerce_order_number` is display-only: it returns `_lwc_order_number` when present and never mutates an order merely because a list or screen was viewed.

An order without a known destination is deliberately **not** numbered yet. The admin "Add new order" screen creates an address-less auto-draft on page load, so its heading initially shows the WooCommerce internal number (for example `Order #191`). The **Order Number** side metabox explains this and previews the next Local and Global IDs. When the administrator enters the shipping or billing address and clicks **Create/Save**, the same order is assigned the configured ID; Indonesia becomes Local and every other country becomes Global. This delay prevents an international order from being prematurely frozen as Local. A real non-draft order with no stored country may use the store base country as a fallback.

The ID is stored in `_lwc_order_number` plus `_lwc_order_number_scope`; an order is never automatically renumbered once it carries an ID. Orders created before the feature was activated keep their original number because `lwc_order_number_enabled_at` gates automatic assignment. Installations that enabled the switch before that timestamp existed self-heal it with a one-day grace window so open/recent order drafts remain eligible but historical orders do not. The settings screen previews the next ID of each scope without consuming a number.

Both automatic generation and manual correction call `order_id_exists()`, which searches the HPOS `wc_orders_meta` table and classic `postmeta`. If resetting/toggling the counter would build an already-used ID, the generator reserves the next sequence until the result is unique. Existing duplicate data is not silently rewritten.

**Manual correction** — `LWC_Order_Number_Admin` adds an **Order Number** side metabox on every order edit screen (admin-only). It shows the immutable WooCommerce internal ID (database primary key), current custom ID, scope, and an editable **Corrected ID** field. Administrators may correct any order, including historical orders; automatic eligibility restrictions do not block an explicit correction. A duplicate of another order's custom ID is rejected. Saving a non-empty correction re-derives the scope from the destination country and updates the displayed ID without a full page reload. Submitting an empty field keeps the existing custom ID; it never silently clears it.
...
- **Customer-selected FedEx service** — displays the International Priority or International Economy service stored with the order at checkout. AWB creation always uses that stored choice; the administrator does not need to select it again and cannot accidentally override it. Older orders without the stored service retain the Economy fallback;
- **Scheduled ship date** — defaults to today and can be set up to 10 days ahead for FedEx Express Future Day. A future date sends `shipDatestamp` plus `requestedShipment.pickupDetail` with `FUTURE_DAY`, allowing the AWB to be created now while the parcel is held until its scheduled tender date;
- **FedEx label item description** — defaults to the configurable FedEx Settings value `AROMATHERAPY ESSENTIAL OILS PURE 100%` and is editable for each AWB, including replacements. Selected items are aggregated into one customs commodity with this single description, total quantity, declared value, and weight, so FedEx derives one standard-label `DESC` field;
...
Shipment creation sends a complete REST v1 payload: shipper/recipient contacts and full street addresses, ship date, `shippingChargesPayment` (SENDER + account), PDF label specification, and — for international destinations — `customsClearanceDetail` with one commodity aggregating all selected order/package products. Its description is the configurable/default AWB text, not concatenated product names; its quantity, customs value, and weight are the totals of the selected products, and its unit price is calculated from the total value and quantity. The country of manufacture uses the store base country. FedEx derives one standard-label `DESC` field from this commodity. The carton/package still uses the actual manual weight and dimensions when supplied, otherwise product-derived weight and volume; this is the data FedEx uses for the physical shipment and rate. The FedEx Settings page stores the default label description; the order metabox permits a per-AWB override. Duties payment defaults to RECIPIENT (`lwc_fedex_duties_payment_type` filter) and terms of sale defaults to DAP (`lwc_fedex_terms_of_sale` filter). Shipment creation uses the service selected by the buyer at checkout; the compact order-list action and order metabox both use that same stored choice automatically, while older orders without the metadata fall back to Economy. Package weight comes from the shipping option selected at checkout unless an actual-carton override is supplied. Before label creation, the order metabox optionally accepts an actual packed-carton weight and a complete length/width/height set in centimeters. Any supplied values override product-derived estimates and intentionally produce one package line item; partial or nonpositive measurements are rejected server-side. The label PDF is stored in uploads as `YYYYMM_LABEL_FEDEX_Customer Name.pdf`, the tracking number saved to `_lwc_fedex_tracking_number`, and an order note added. A duplicate customer/month filename gets a numbered suffix rather than overwriting an earlier label. Shipper contact details come from the `lwc_fedex_shipper_name` and `lwc_fedex_shipper_phone` options (phone required by FedEx).
...
### Automatic AWB creation policy

One module owns the decision. `LWC_AWB_Automation` hooks `woocommerce_order_status_processing` and asks `LWC_Courier_Registry::courier_for_order()` which courier the order uses; the per-courier integrations no longer register their own creation hooks.

- **FedEx is manual only.** Its AWB carries the scheduled ship date, the actual carton weight and dimensions, and the printed label description (`DESC1`). FedEx bakes those values into the label while the AWB is created and the Ship API has no update-label call, so creating the AWB on order receipt would freeze values the merchant has not filled in yet. Receiving a FedEx order therefore only captures the buyer's service type and the tariff on the order's shipping line (`lwc_fedex_service`, `lwc_fedex_max_weight`, and the line total); the AWB and label wait until the merchant sets the items, carton, ship date, and label text on the order screen.
- **Every other courier keeps creating its AWB automatically** on that same status transition: `jt_express` through `LWC_JT_Order_Admin::create_on_processing()`, and `rayspeed` through `LWC_RaySpeed_Order_Admin::auto_create_awb()` (which also stores `_lwc_rayspeed_create_error` when the carrier rejects the request instead of interrupting the status change).
- `jt_cargo` has no order-creation call in this plugin yet and its rates are hidden from checkout, so it has no automatic path; `jne` belongs to the JNE plugin.
- The manual-only courier list is filterable (`lwc_awb_manual_only_couriers`) and the per-courier automation decision is filterable (`lwc_awb_should_auto_create`). The FedEx exclusion is applied before the second filter, so it cannot be switched back on by accident.
...
- **`lwc_jt_cargo`** (`LWC_Shipping_JTC`) — reserved large/heavy shipping method implemented in its own `shipping/jtc` provider module. While API coordination is unfinished it remains internal-only: the class and settings exist for development, but the method is not registered with WooCommerce and Cargo rates are removed defensively from checkout/admin quotes.
- **J&T Cargo provider boundary** — `LWC_JTC_Account` owns Cargo credentials and `LWC_JTC_API` owns its 20 supplied Sandbox interface addresses plus a non-destructive location-endpoint health check. Neither class inherits from or calls the J&T Express account/API/shipping classes. Production has no guessed host and remains disabled until `lwc_jtc_production_base_url` or environment endpoint filters provide coordinated URLs.
- **J&T Cargo Sandbox console** — the Cargo provider page has a separate administrator-only Sandbox Test tab covering all 20 whitelisted demo interfaces. It accepts JSON payloads, optional JSON headers, GET/POST, and JSON/form bodies, then shows HTTP status, latency, and the decoded/raw carrier response. The URL is never user-editable and requests cannot target Production or arbitrary hosts.
- Each Sandbox interface tracks attempts and requires three HTTP 2xx responses (`3/3`) for local completion. Progress is bound to the active Sandbox UUID and resets logically when that UUID changes. Local completion is explicitly separate from Production approval, which remains an external J&T Cargo coordination decision.

The providers have no shared account, API, settings form, or shipping superclass. J&T Express is an active domestic method with all-Indonesia or Java-only coverage; Cargo is an isolated internal test module. The legacy `lwc_jt` method id aliases only to Express so existing zone instances keep working. Express stores its distinct Order, Tariff, Tracking, Print, and Cancellation credentials. Cargo independently stores username, interface UUID, API key, and API secret for each environment; its 20 Sandbox endpoints are fixed internally, while Production URLs remain empty until coordinated. Legacy pre-split J&T credentials migrate only to Express.
...
Managed-provider orders have a **Change Shipping** metabox. Orders that are still `on-hold` or `pending` may switch before fulfillment. An active J&T shipment must first be cancelled by the API or confirmed by carrier tracking status 162/163. Active FedEx shipments must first be cancelled through the FedEx Ship API from the order metabox; RaySpeed and provisional J&T Cargo require explicit administrator confirmation when an existing AWB was cancelled externally because the plugin has no shipment-void endpoint for those providers. Cancelling a pickup alone is insufficient. Replacement choices are recalculated from the order address and products and are restricted to currently available LoveCatz methods plus the official JNE method (`jneshof_shipping`) when that plugin is active and its matching WooCommerce zone returns live rates. After selecting a service, the administrator must run **Check rate** and review the quoted service and cost; **Change Shipping** remains disabled until a short-lived server-side quote exists, and submission revalidates the destination, service, and price. The selected rate's title, method/instance IDs, cost, taxes, and carrier metadata replace the old shipping item, totals are recalculated, previous fulfillment data is archived, and the new AWB must be created manually. A generation suffix prevents a replacement J&T shipment from reusing its prior `orderid`.
...
- Options: `lwc_menu_*`, `lwc_enable_product_quantity_limits`, `lwc_enable_product_preorder`, `lwc_preorder_all_products`, `lwc_preorder_product_ids`, `lwc_jt_{express|cargo}_*` (plus legacy `lwc_jt_*`), `lwc_fedex_*` (including `lwc_fedex_shipper_name` and `lwc_fedex_shipper_phone`), `lwc_currency_enabled`, `lwc_currency_rates`, `lwc_international_fee_*`, `lwc_order_number_*` (plus `lwc_order_number_seq_{local|global}[_Ymd]` counters), rewrite-version, and legacy promo options.
...
- Order meta: `_lwc_fedex_label_path`, `_lwc_fedex_tracking_number`, `_lwc_jt_awb`, `_lwc_jt_order_id`, `_lwc_jt_etd`, `_lwc_jt_tracking`, `_lwc_jt_create_error`, `_lwc_jt_tracking_error`, `_lwc_jt_cancelled`, `_lwc_order_number`, `_lwc_order_number_scope`.
...
- Verify international FedEx payloads aggregate selected items into exactly one commodity using the configured description and totals for quantity, declared value, and weight, producing one label `DESC` field; package weight and dimensions must remain the actual/manual carton values when supplied or product-derived values otherwise.
